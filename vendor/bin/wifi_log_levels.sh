...
METRICS_DEFAULT_RSSI_THD=(-85 -80 -75 -70 -63 -55 -40 -20) # Array of thresholds for RSSILevel and NoiseLevel metrics
currentLoop=0
...
{
    WLAN_FREQ=`$IW $WLAN_INTERFACE link | $GREP "freq: " | $GREP -Eo '[0-9]+'`
    CHANNEL=`$IWPRIV $WLAN_INTERFACE driver get_cnm | $GREP channels | cut -d " " -f4`
    # Wi-Fi 6E channel numbers collide with 2.4|5GHz channel numbers. Add 6e
    # prefix for Wi-Fi 6E channels to disambiguate.
    if [[ ( "$WLAN_FREQ" -ge "5925" ) && ( "$WLAN_FREQ" -le "7125" ) ]] ; then
        CHANNEL="6e-"${CHANNEL}
    fi
}


function kdm_rssi_noise_level()
{
    if [[ "$1" -lt "${RSSINOISETHD[0]}" ]]; then
        BARLEVEL=0
    elif [[ "$1" -lt "${RSSINOISETHD[1]}" ]]; then
        BARLEVEL=1
    elif [[ "$1" -lt "${RSSINOISETHD[2]}" ]]; then
        BARLEVEL=2
    elif [[ "$1" -lt "${RSSINOISETHD[3]}" ]]; then
        BARLEVEL=3
    elif [[ "$1" -lt "${RSSINOISETHD[4]}" ]]; then
        BARLEVEL=4
    elif [[ "$1" -lt "${RSSINOISETHD[5]}" ]]; then
        BARLEVEL=5
    elif [[ "$1" -lt "${RSSINOISETHD[6]}" ]]; then
        BARLEVEL=6
    elif [[ "$1" -lt "${RSSINOISETHD[7]}" ]]; then
        BARLEVEL=7
    else
        BARLEVEL=8
    fi
...
function log_kdm_rssi_level
{
    ...
    kdm_rssi_noise_level ${maxRssi}
    beacon_rssibar=$BARLEVEL

    kdm_rssi_noise_level ${dataRssi}
    rssibar=$BARLEVEL

    ## Dual-chain metrics are only enabled on Mantis
    if [ $CHIPNSS = "2" ]; then
        kdm_rssi_noise_level ${dataRssi2}
        rssibar2=$BARLEVEL
...
{
    kdm_rssi_noise_level ${maxNoise}
    noisebar=$BARLEVEL
    ## Dual-chain metrics are only enabled on Mantis
    if [ $CHIPNSS = "2" ]; then
        kdm_rssi_noise_level ${maxNoise2}
        noisebar2=$BARLEVEL
...
function log_kdm_band
{
    ...
    if [[ ( "$WLAN_FREQ" -ge "5945" ) && ( "$WLAN_FREQ" -le "7125" ) ]] ; then
        log_to_kdm_minerva "operation=Band;key=${CHANNEL};metadata=${BANDWIDTH};metadata1=${wifimode};metadata2=6e"
    else
...
# Create the array of RSSI and Noise thresholds
RSSINOISETHD=(${METRICS_DEFAULT_RSSI_THD[@]})
