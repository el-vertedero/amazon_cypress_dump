...
#Hoya HVT = 4 mics
#Hoya EVT, DVT, PVT = 6 mics
HOYA_HVT_ID="01D0000100110021"
...
# For Hoya HVT, set mic count to 6 if idme miccal4 and miccal5 are non-zero
if [ $BOARD_ID == $HOYA_HVT_ID ]; then
...
#for Hoya EVT/DVT/PVT set mic count to 6
    if [ -z "${MIC_NUM}" ]; then
        /vendor/bin/setprop persist.asp.mic.count 6
    fi
fi
